export default {
  ...
  storage: {
    ...
    nfsExample: 'NFS示例：vers=3,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2',
    cifsExample: 'CIFS示例：vers=3.0,iocharset=utf8,file_mode=0777,dir_mode=0777',
    provider: '提供商',
    ...
    noBoundNodeForDetails: '该存储未绑定任何节点，无法查看详情。请先为存储分配一个节点。',
    boundNodeNotExists: '绑定的节点不存在，请重新分配节点',
    boundNodeOffline: '绑定的节点 "{nodeName}" 当前不在线，可能无法正常访问存储',
    boundNodeAgentNotRunning: '绑定的节点 "{nodeName}" 的 Agent 未运行，可能无法正常访问存储',
    ...
    testNodeAgentNotRunning: '测试节点 "{nodeName}" 的 Agent 未运行，请确保 Agent 已启动',
    creatingConnectionTestTask: '正在创建连接测试任务，将使用节点: {nodeName} ({ipAddress})',
    connectionTestTaskCreated: '连接测试任务已创建，任务ID: {taskId}',
    testConnectionFailed: '测试连接失败',
    uploadSuccess: '上传成功',
    uploadFailed: '上传失败',
    refreshStatsConfirm: '此动作将会重新调用接口，如果桶数据量较多那么时间会较长，请耐心等待',
    storageDetails: '存储详情',
  ...
  taskWizard: {
    ...
    selectSourceStorageAndFilesDesc: '请选择要同步的源端存储，并勾选需要同步的目录或文件',
  ...
  targetSelector: {
    ...
    configureTargetLocationDesc: '您可以选择浏览现有存储桶或创建新的存储桶来作为同步目标',
    configureTargetLocationNasDesc: '请选择目标目录路径，您可以通过浏览功能查看目录结构',
  ...
  alertPolicies: {
    ...
    selectNotificationChannelsTip: '选择通知发送的渠道，将自动筛选兼容的模板和对象',
    selectAlertTemplateTip: '选择用于发送通知的模板（根据渠道类型自动筛选）',
    selectNotificationTargetsTip: '选择接收通知的目标对象（根据渠道类型自动筛选）',
    policyDetails: '策略详情',
    update: '更新',
    create: '创建',
    alertConfig: '告警配置',
  ...
  settings: {
    ...
    messages: {
      ...
      confirmResetNotificationSettings: '确定要重置所有通知设置吗？此操作不可恢复。',
      warning: '警告',
  ...
  nodes: {
    serverIp: '服务地址',
    serverPort: '服务器端口',
    serverIpTip: '服务器IP地址是EasySync-Proxy可以连接的EasySync服务端的服务地址',
    serverPortTip: '服务器端口是EasySync-Proxy可以连接的EasySync服务端的端口',
    serverIpPlaceholder: '请输入EasySync服务端的服务地址',
    serverPortPlaceholder: '请输入EasySync服务端的端口',
    // 节点分组和标签
    noGroup: '未分组',
    batchGroup: '批量分组',
    ...
    step3Title: '配置同步任务',
    step3Description: '选择任意两端存储和代理节点，系统自动分配任务，支持双向同步',
    step4Title: '执行与监控',
  ...
  clients: {
    ...
    sourceServerDesc: '用户的数据源服务器，安装Agent后可获取本地分区、目录、文件信息',
  ...
  taskWizard: {
    ...
    createCopy: '创建副本任务',
    selectSourceStorageAndFiles: '选择源端存储和文件',
    selectSourceStorageAndFilesDesc: '请选择源端存储，并选择需要同步的文件或目录',
    copyTaskConfig: '复制任务配置',
