...
"""
    ...
    def _create_dual_rclone_config(self, source_config: Dict[str, Any], target_config: Dict[str, Any]) -> tuple:
        ...
        # 获取源OBS配置
        source_provider = source_config_inner.get('provider', 'AWS')
        ...
        # 优先使用任务配置中的 endpoint（如果存在），否则使用存储配置
        source_endpoint = source_config.get('endpoint') or source_config_inner.get('endpoint', 'obs.cn-north-4.myhuaweicloud.com')

        # 获取目标OBS配置
        target_provider = target_config_inner.get('provider', 'AWS')
        ...
        # 优先使用任务配置中的 endpoint（如果存在），否则使用存储配置
        target_endpoint = target_config.get('endpoint') or target_config_inner.get('endpoint', 'obs.cn-north-4.myhuaweicloud.com')

        # 规范化 S3 endpoint：去除 /login 等 Console 路径；IP/localhost 服务
        # 默认使用 path-style，避免 MinIO 等兼容服务无法解析虚拟主机名。
        def normalize_s3_endpoint(endpoint: str, configured_path_style: bool):
            raw = str(endpoint or '').strip().rstrip('/')
            parsed = urlparse(raw if '://' in raw else f'https://{raw}')
            hostname = (parsed.hostname or '').lower()
            clean_endpoint = urlunparse((parsed.scheme, parsed.netloc, '', '', '', ''))
            ipv4 = hostname.count('.') == 3 and all(part.isdigit() for part in hostname.split('.'))
            is_local = hostname in {'localhost', '127.0.0.1', '::1'} or ipv4
            return clean_endpoint, bool(configured_path_style or is_local)

        source_endpoint, source_path_style = normalize_s3_endpoint(
            source_endpoint, source_config_inner.get('path_style', False)
        )
        target_endpoint, target_path_style = normalize_s3_endpoint(
            target_endpoint, target_config_inner.get('path_style', False)
        )
        ...
        # 虚拟托管样式：endpoint（不包含bucket）
        # 路径样式：endpoint（不包含bucket）
        # 注意：rclone会自动处理虚拟托管样式，endpoint不需要包含bucket名称
        # 源端 endpoint 处理
        if not source_path_style:
            # 虚拟托管样式下，endpoint 不包含 bucket 名称，rclone会自动处理
            self.logger.debug(f"源端虚拟托管样式：endpoint 保持不变 {source_endpoint}")
        elif source_path_style:
            # 路径样式下，endpoint 不包含 bucket 名称
            self.logger.debug(f"源端路径样式：endpoint 保持不变 {source_endpoint}")

        # 目标端 endpoint 处理
        if not target_path_style:
            # 虚拟托管样式下，endpoint 不包含 bucket 名称，rclone会自动处理
            self.logger.debug(f"目标端虚拟托管样式：endpoint 保持不变 {target_endpoint}")
        elif target_path_style:
            # 路径样式下，endpoint 不包含 bucket 名称
            self.logger.debug(f"目标端路径样式：endpoint 保持不变 {target_endpoint}")

        # 判断是否需要 region 配置
        # 不管是路径样式还是虚拟托管样式，都需要 region 配置
        source_use_region = source_region is not None and source_region != ''
        target_use_region = target_region is not None and target_region != ''
...
secret_access_key = {source_secret_key}"""
        ...
        # 根据 endpoint 格式决定是否添加 region
        if source_use_region:
            config_content += f"\nregion = {source_region}"
        else:
            # 如果 endpoint 包含 bucket 名称，移除 region 配置
            pass

        config_content += f"\nendpoint = {source_endpoint}"
        config_content += f"\nforce_path_style = {'true' if source_path_style else 'false'}"
        config_content += f"""
acl = private
storage_class = STANDARD
...
secret_access_key = {target_secret_key}"""
        ...
        config_content += f"\nforce_path_style = {'true' if target_path_style else 'false'}"
        config_content += f"""
acl = private
storage_class = STANDARD
"""
        ...
        try:
            ...
            try:
                ...
                if options:
                    ...
                    # 处理不创建空目录
                    if options.get('no_create_empty_dirs', False):
                        cmd.extend(['--no-create-empty-dirs'])
        ...
        try:
            ...
            try:
                ...
                if source_path_style:
                    # 路径样式：在路径中指定 bucket
                    # 如果 source_path 为空或等于 bucket 名称，使用 bucket 根目录
                    if not source_path or source_path == source_bucket:
                        source_full_path = f"{source_remote_name}:{source_bucket}"
                    else:
                        source_full_path = f"{source_remote_name}:{source_bucket}/{source_path}"
                else:
                    # 虚拟托管样式：endpoint 不包含 bucket 名称，bucket 已经在 endpoint 中
                    # 如果 source_path 为空或等于 bucket 名称，使用 bucket 根目录
                    # 注意：虚拟托管样式下，仍然需要指定bucket名称
                    if not source_path or source_path == source_bucket:
                        source_full_path = f"{source_remote_name}:{source_bucket}"
                    else:
                        source_full_path = f"{source_remote_name}:{source_bucket}/{source_path}"
                ...
                # 构建目标端路径
                # 注意：无论 path_style 是什么（虚拟托管样式或路径样式），
                # rclone 的路径格式永远都是 remote:bucket/path，
                # path_style 只影响 HTTP 请求的 URL 格式，不影响 rclone 路径语法。
                # 之前代码在虚拟托管样式下错误地省略了 bucket 名称，
                # 导致 rclone 命令变成 sync source:bucket target:，
                # rclone 会尝试扫描目标端所有存储桶，导致任务卡死无进度。
                target_bucket = ''
                ...
                # rclone 路径必须包含 bucket 名称，与 path_style 无关
                if not target_path or target_path == target_bucket:
                ...
                if process.returncode != 0:
                    ...
                    self.logger.error(f"同步失败: {error_output}")
                    raise subprocess.CalledProcessError(process.returncode, cmd)
