...
class EnhancedProgressTracker:
    ...
    def __init__(self, server_comm, config: Dict[str, Any]):
        ...
        # 配置参数
        self.report_interval = config.get('progress', {}).get('report_interval', 5)  # 5秒上报一次
        self.detailed_logging = config.get('progress', {}).get('detailed_logging', True)
        self.max_history_size = config.get('progress', {}).get('max_history_size', 100)
    ...
    def _report_progress_to_server(self, progress: TaskProgress):
        ...
        try:
            ...
            # 发送给服务器
            self.server_comm.update_task_status(progress.task_id, {
                'status': 'running',
                'progress': progress.percentage,
                'details': report_data
            })
